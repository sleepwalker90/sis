    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            x-data="{ 
                    ...
                    dormitoryId: '',
                    dormitoryName: '',
                    openConfirmationModal(dormitoryId, dormitoryName) {
                        ...
                        <tbody>
                            @foreach ($dormitories as $dormitory)
                                    ...
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td class="px-3 py-4">
                                                <div class="font-semibold text-black"><a href="{{ route('admin.dormitories.index', ['dormitory' => $dormitory->id] ) }}">{{ $dormitory->building }}</a></div>
                                        </td>
                                        <td class="px-3 py-4">
                                                <div class="font-semibold text-black"><a href="{{ route('admin.dormitories.index', ['dormitory' => $dormitory->id] ) }}">{{ $dormitory->rooms->sum('occupied_beds') }}/{{ $dormitory->rooms->sum('beds') }}</a></div>
                                        </td>
                                        <td class="px-3 py-4">
                                                <div class="font-semibold text-black">{{ $dormitory->rooms->where('occupied_beds', 0)->count() }}/{{ $dormitory->rooms->count() }}</div>
                                        </td>
                        ...
                        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                            ...
                            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" x-show="showConfirmation" @click.away="showConfirmation = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-50" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-50">
                                        ...
                                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                            ...
                                            <div class="mt-2">
                                                <p class="text-sm text-gray-500">
                                                    Are you sure you want to delete dormitory <strong x-text="dormitoryName"></strong>? This action cannot be undone.
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                    <button type="button" x-on:click="document.getElementById(dormitoryId).submit()" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">
                                        Delete
                                    </button>
