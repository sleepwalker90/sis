<x-admin-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8" x-data="{
            ...
            translations: {
                approve_message: '{{ __('Are you sure you want to approve this application?') }}',
                deny_message: '{{ __('Are you sure you want to deny this application?') }}'
            },
            openUpdateApplicationModal(applicationId, status) { 
                this.currentApplicationId = applicationId; 
                this.updateStatus = status; 
                this.updateMessage = status === 'approved' ? this.translations.approve_message : this.translations.deny_message; 
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mt-4">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="p-3">{{ __('Faculty Number') }}</th>
                                <th scope="col" class="p-3">{{ __('Status') }}</th>
                                <th scope="col" class="p-3 text-center">{{ __('Actions') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-3 py-4 text-center">
                                        <div class="flex items-center justify-center space-x-4">
                                            <button class="font-medium bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-md" type="button" x-on:click="openUpdateApplicationModal({{ $application->id }}, 'approved')">{{ __('Approve') }}</button>
                                            <button class="font-medium bg-red-500 hover:bg-red-600 text-white py-2 px-4 rounded-md" type="button" x-on:click="openUpdateApplicationModal({{ $application->id }}, 'denied')">{{ __('Deny') }}</button>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-4">{{ __('No pending applications') }}.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div x-show="showUpdateApplicationModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" x-cloak>
                <div class="bg-white rounded-lg shadow p-8">
                    <h2 class="text-xl font-semibold mb-4">{{ __('Update Application Status') }}</h2>
                    ...
                    <form method="POST" :action="`/admin/dormitory-applications/${currentApplicationId}`">
                        ...
                        <div class="py-6">
                            ...
                            <x-secondary-button type="button" x-on:click="showUpdateApplicationModal = false">{{ __('Cancel') }}</x-secondary-button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div x-show="showBulkApproveModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" x-cloak>
                <div class="bg-white rounded-lg shadow p-8">
                    <h2 class="text-xl font-semibold mb-4">{{ __('Bulk Approve Applications') }}</h2>
                    <p class="mb-4">{{ __('Are you sure you want to approve all pending applications and accommodate students') }}?</p>
                    <form method="POST" action="{{ route('admin.dormitory-applications.bulk-approve') }}">
                        @csrf

                        <div class="py-6">
                            <x-primary-button>{{ __('Yes, Approve All') }}</x-primary-button>
                            <x-secondary-button type="button" x-on:click="showBulkApproveModal = false">{{ __('Cancel') }}</x-secondary-button>
                        </div>
                    </form>
                </div>
            </div>
