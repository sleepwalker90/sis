<x-lecturer-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Attestations for stream') . ' ' . $assignment->stream->number . ' ' . __('in') . ' ' . $assignment->course->name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('lecturer.attestations.store', $assignment) }}">
                    @csrf
                    <table class="min-w-full bg-white dark:bg-gray-800 border rounded-sm border-separate">
                        <thead>
                            <tr>
                                ...
                                <th class="px-4 py-2 border">{{ __('Faculty number') }}</th>
