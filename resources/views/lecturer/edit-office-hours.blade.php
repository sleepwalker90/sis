        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('lecturer.update-office-hours') }}">
                    ...
                    <div class="mb-4">
                        <label for="time_interval" class="block text-sm font-medium text-gray-700">{{ __('Time Interval') }}</label>
                        <input type="text" id="time_interval" name="time_interval" class="block w-full mt-1 shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="{{ __('e.g.') }}, 10:00-12:00" required>
                        <small class="text-gray-600">{{ __('Format') }}: HH:MM-HH:MM</small>
                    </div>
                    ...
                    <div class="flex justify-end">
                        <x-primary-button>{{ __('Add Office Hours') }}</x-primary-button>
                    </div>
                </form>
                @if (isset($lecturer->office_hours))
                    <h3 class="mt-6 text-lg font-medium text-gray-900 dark:text-gray-200">{{ __('Current Office Hours') }}:</h3>
                    <ul class="mt-4">
                        
                            @foreach(json_decode($lecturer->office_hours, true) ?? [] as $day => $hours)
                            <li class="mb-2">
                                <span class="font-semibold">{{ __(ucfirst($day)) }}: </span>{{ $hours }}
                                <form method="POST" action="{{ route('lecturer.remove-office-hours') }}" class="inline">
                                    @csrf
                                    <input type="hidden" name="day" value="{{ $day }}">
                                    <button type="submit" class="text-red-500 ml-2">{{ __('Remove') }}</button>
                                </form>
                            </li>
                            @endforeach
